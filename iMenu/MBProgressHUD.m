...
-(void) cancelAlertHud: (id)sender
{ 
    if(delegate != nil) 
    {
        if ([delegate respondsToSelector:@selector(clearHud)]) 
        {
            [delegate performSelector:@selector(clearHud) withObject:self];
        }
    }
}
